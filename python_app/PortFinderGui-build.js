...
export async function buildApp() {
  ...
  const nuitkaArgs = [
    path.join(SCRIPT_DIR, 'PortFinderGui.py'),
    '--output-file=PortFinderGui.exe',
    '--report=build/nuitka-PortFinderGui-report.xml',
    '--output-dir=build',
    `--windows-icon-from-ico=${ICON}`,
    `--windows-company-name=${COMPANY_NAME}`,
    '--windows-product-name=PortFinderGui',
...
if (process.argv.some((arg) => arg.includes('PortFinderGui-build.js'))) {
