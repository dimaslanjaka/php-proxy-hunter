...
class ParallelCheckerWorkerThread(QThread):
    """Background worker thread for parallel proxy checking"""

    result_signal = Signal(object)
    finished_signal = Signal()
    status_signal = Signal(str)

    def __init__(self, proxies, checker_instance, max_workers):
        super().__init__()
    ...
    def run(self):
        """Run parallel proxy checking using ThreadPoolExecutor"""
        ...
        try:
            self.checker._active_count = self.max_workers

            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                # Submit all proxy checks to the executor
                futures = {
                    executor.submit(self.checker.check_proxy, proxy): proxy
                ...
                for future in as_completed(futures):
                    ...
                    if (
                        self.checker._cancel_event
                        and self.checker._cancel_event.is_set()
                    ):
...
class ParallelProxyChecker(QWidget):
    # Emitted from background threads to update the UI safely. Emits a single Proxy object.
    result_signal = Signal(object)
    finished_signal = Signal()
    status_signal = Signal(str)
    ...
    def __init__(self):
        ...
        self.setWindowTitle("Parallel Proxy Checker - DL Traffic")
        ...
        saved = load_text("parallel_proxy_text_manual")
        if saved:
        ...
        saved = load_text("parallel_proxy_text_untested")
        if saved:
        ...
        saved = load_text("parallel_proxy_text_working")
        if saved:
        ...
        # Buttons and settings layout
        buttons_layout = QHBoxLayout()

        self.check_button = QPushButton("Check Proxies (Parallel)")
        ...
        buttons_layout.addWidget(workers_label)

        self.workers_spinbox = QSpinBox()
        self.workers_spinbox.setMinimum(1)
        self.workers_spinbox.setMaximum(32)
        # Load saved workers count or default to 8
        saved_workers = load_text("parallel_workers_count")
        default_workers = 8
        try:
            default_workers = int(saved_workers) if saved_workers else 8
        except (ValueError, TypeError):
            default_workers = 8
        self.workers_spinbox.setValue(default_workers)
        # Save value whenever spinbox changes
        self.workers_spinbox.valueChanged.connect(
            lambda value: save_text("parallel_workers_count", str(value))
        )
        buttons_layout.addWidget(self.workers_spinbox)
    ...
    def fetch_untested_proxies(self):
        ...
        try:
            ...
            save_text("parallel_proxy_text_untested", proxy_list)
        except Exception as e:
    ...
    def fetch_working_proxies(self):
        ...
        try:
            ...
            save_text("parallel_proxy_text_working", proxy_list)
        except Exception as e:
    ...
    def check_proxy(self, data: Proxy):
        """Check a single proxy (can be called in parallel)"""
        ...
        # Use a fresh database connection for thread safety
        db = init_readonly_db()
        try:
    ...
    def run_checker(self):
        """Parse proxies and start background parallel checking thread"""
        ...
        if active_tab_index == 0:
            save_text("parallel_proxy_text_manual", active_tab.toPlainText())
        elif active_tab_index == 1:
            save_text("parallel_proxy_text_untested", active_tab.toPlainText())
        elif active_tab_index == 2:
            save_text("parallel_proxy_text_working", active_tab.toPlainText())
    ...
    def _parse_and_start_check(self, parse_func):
        """Parse proxies and start the parallel checking process"""
        proxies = parse_func()
        ...
        self._completed = 0
        self._active_count = self.workers_spinbox.value()
        self._start_time = time.time()
        self._cancel_event = threading.Event()

        # Create and start worker thread with parallel execution
        self.worker_thread = ParallelCheckerWorkerThread(
            proxies, self, self.workers_spinbox.value()
        )
