    ...
    def __init__(self, method, proxy):
        if method.lower() not in ["http", "https", "socks4", "socks5"]:
            raise NotImplementedError(
                "Only HTTP, HTTPS, SOCKS4, and SOCKS5 are supported"
            )
    ...
    def check(self, site, timeout, user_agent, verbose):
        if self.method in ["socks4", "socks5"]:
            ...
            try:
                ...
                verbose_print(
                    verbose, f"Proxy {self.proxy} is valid, time taken: {time_taken}"
                )
                return True, time_taken, None
            except Exception as e:
                verbose_print(
                    verbose, f"Proxy {self.proxy} is not valid, error: {str(e)}"
                )
                return False, 0, e
        else:
...
def main():
    ...
    parser.add_argument(
        "-p",
        "--proxy",
        help="Check HTTPS, HTTP, SOCKS4, or SOCKS5 proxies",
        default="http",
    )
    parser.add_argument(
        "-l", "--list", help="Path to your proxy list file", default="tmp/output.txt"
    )
    parser.add_argument(
        "-s",
        "--site",
