...
import com.dimaslanjaka.proxyhunter.service.LocalSharedPrefs
...
@Composable
fun ProxyListScreen(onConnect: (ProxyItem) -> Unit, onDisconnect: () -> Unit) {
    ...
    LaunchedEffect(Unit) {
        val pref = try { LocalSharedPrefs.initialize(context, "proxy") } catch (e: Exception) { null }
...
fun ProxyListScreenPreview() {
    ProxyHunterTheme {
        ProxyListScreen(onConnect = {}, onDisconnect = {})
    }
}
