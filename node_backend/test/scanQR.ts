...
if (!fs.existsSync('./baileys_auth_info')) fs.mkdirSync('./baileys_auth_info');
...
const startSock = async () => {
  ...
  if (useMobile && !sock.authState.creds.registered) {
    ...
    const mcc = (PHONENUMBER_MCC as any)[phoneNumber.countryCallingCode];
    if (!mcc) {
      throw new Error(
        'Could not find MCC for phone number: ' + registration!.phoneNumber + '\nPlease specify the MCC manually.'
      );
    }
  ...
  const sendMessageWTyping = async (msg: pkg.AnyMessageContent, jid: string) => {
      ...
      if (events['contacts.update']) {
        for (const contact of events['contacts.update']) {
          if (typeof contact.imgUrl !== 'undefined') {
            const newUrl =
              contact.imgUrl === null ? null : await sock!.profilePictureUrl(contact.id!).catch(() => null);
  ...
  async function getMessage(key: pkg.WAMessageKey): Promise<pkg.WAMessageContent | undefined> {
    if (store) {
