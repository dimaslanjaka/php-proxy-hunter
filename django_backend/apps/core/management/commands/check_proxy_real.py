...
def worker(item: Dict[str, str]):
    ...
    try:
        ...
        if test["result"]:
            ...
            sql_exec(
                f"UPDATE proxies SET status = 'active', type = '{protocols}', https = '{https}', last_check = strftime('%Y-%m-%dT%H:%M:%SZ', 'now') WHERE proxy = '{item['proxy']}';"
            )
        else:
            if db:
                db.update_status(item["proxy"], "dead")
            sql_exec(
                f"UPDATE proxies SET status = 'dead' WHERE proxy = '{item['proxy']}';"
            )
