...
class TestUser(unittest.TestCase):
    ...
    def test_1_create(self):
        ...
        self.headers.update(
            {
                ...
                "X-CSRFToken": utils.parse_csrf_token_from_cookie_file(
                    self.admin.cookie_file
                ),
            }
        )
        response = build_request(
            ...
            method="POST",
            cookie_file=self.admin.cookie_file,
        )
    ...
    def test_4_logout(self):
        ...
        response = build_request(
            endpoint=url, cookie_file=self.admin.cookie_file, headers=self.headers
        )
