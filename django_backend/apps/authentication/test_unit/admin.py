...
class TestAdmin(unittest.TestCase):
    ...
    def test_2_login(self):
        ...
        login_data = {"username": self.admin_username, "password": self.admin_password}
        self.headers.update(
        ...
        self.assertTrue(
            "Welcome, admin" in response.text or "Login success" in response.text
        )

    def test_3_status(self):
        url = "http://127.0.0.1:8000/auth/status"
        response = build_request(
            endpoint=url, cookie_file=self.cookie_file, headers=self.headers
        )
